...
import io.metersphere.platform.client.FullGoalClient;
 ...
 * 业务实现类
 */
public class FullGoalPlatform extends AbstractPlatform {

    protected FullGoalClient fullGoalClient;

    protected SimpleDateFormat sdfWithZone = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");

    protected Boolean isSass = false;

    private Set<String> fullGoalImageFileNames;

    private static final String ATTACHMENT_NAME = "attachment";

    public FullGoalPlatform(PlatformRequest request) {
        super.key = FullGoalPlatformMetaInfo.KEY;
        super.request = request;
        fullGoalClient = new FullGoalClient();
    }

    @Override
    public IssuesWithBLOBs addIssue(PlatformIssuesUpdateRequest request) {
        Map addJiraIssueParam = buildUpdateParam(request);
        FullGoalAddIssueResponse result = fullGoalClient.addIssue(JSON.toJSONString(addJiraIssueParam));
        FullGoalIssue jiraIssue = fullGoalClient.getIssues(result.getId());

        // 上传富文本中的图片作为附件
        List<File> imageFiles = getImageFiles(request);
        imageFiles.forEach(img -> fullGoalClient.uploadAttachment(result.getKey(), img));
     ...
     *
     * @param issue
     * @param fullGoalEfficiencyIssue
     * @return
     */
    private PlatformIssuesDTO getUpdateIssue(PlatformIssuesDTO issue, FullGoalIssue fullGoalEfficiencyIssue) {
        try {
            if (issue == null) {
            ...
            Map fields = fullGoalEfficiencyIssue.getFields();
            ...
            // todo check
            List<PlatformCustomFieldItemDTO> customFieldItems = syncIssueCustomFieldList(issue.getCustomFieldList(), fullGoalEfficiencyIssue.getFields());
    ...
    public SyncIssuesResult syncIssues(SyncIssuesRequest request) {
        ...
        issues.forEach(item -> {
            try {
                FullGoalIssue jiraIssue = fullGoalClient.getIssues(item.getPlatformId());
...
//        return fullGoalClient.getIssueType(projectConfig.getJiraKey())
//                .stream()
...
//
//        try {
//            Map<String, JiraCreateMetadataResponse.Field> createMetadata = fullGoalClient.getCreateMetadata(projectConfig.getJiraKey(), projectConfig.getJiraIssueTypeId());
//            List<JiraUser> userOptions = fullGoalClient.getAssignableUser(projectConfig.getJiraKey());
//
...
//    }
    ...
    private Map<String, Object> buildUpdateParam(PlatformIssuesUpdateRequest request) {
...
//        validateIssueType();
//        fullGoalImageFileNames = new HashSet<>();
//
//        Map param = buildUpdateParam(request, projectConfig.getJiraIssueTypeId(), projectConfig.getJiraKey());
//        fullGoalClient.updateIssue(request.getPlatformId(), JSON.toJSONString(param));
//
//        // 同步Jira富文本有关的附件
...
//            try {
//                List<JiraTransitionsResponse.Transitions> transitions = fullGoalClient.getTransitions(request.getPlatformId());
//                transitions.forEach(transition -> {
//                    if (Objects.equals(request.getPlatformStatus(), transition.getTo().getName())) {
//                        fullGoalClient.setTransitions(request.getPlatformId(), transition);
//                    }
//                });
//            } catch (Exception e) {
//                LogUtil.error(e);
...
//        fullGoalClient.deleteIssue(platformId);
//    }
//
//    @Override
//    public void validateIntegrationConfig() {
...
//            JiraIssueProject project = fullGoalClient.getProject(projectConfig.getJiraKey());
...
//        return getPlatformStatus(fullGoalClient.getStatus());
//    }
//
...
//        return getPlatformStatus(fullGoalClient.getTransitions(issueKey));
//    }
//
...
//                fullGoalClient.getCreateMetadata(projectConfig.getJiraKey(), projectConfig.getJiraIssueTypeId());
//
...
//        List<JiraUser> userOptions = fullGoalClient.getAssignableUser(projectKey);
//        List options = new ArrayList();
...
//        return fullGoalClient.proxyForGet(path, responseEntityClazz);
//    }
    ...
    public void syncIssuesAttachment(SyncIssuesAttachmentRequest request) {
        ...
        if (StringUtils.equals(AttachmentSyncType.UPLOAD.syncOperateType(), syncType)) {
            // 上传附件
            fullGoalClient.uploadAttachment(request.getPlatformId(), file);
        } else if (StringUtils.equals(AttachmentSyncType.DELETE.syncOperateType(), syncType)) {
            // 删除附件
            FullGoalIssue jiraIssue = fullGoalClient.getIssues(request.getPlatformId());
            ...
            if (!attachments.isEmpty() && attachments.size() > 0) {
                for (int i = 0; i < attachments.size(); i++) {
                    ...
                    if (filename.equals(file.getName())) {
                        ...
                        fullGoalClient.deleteAttachment(fileId);
                    }
                }
            }
        }
    ...
    public void syncJiraIssueAttachments(SyncIssuesResult syncIssuesResult, PlatformIssuesDTO issue, FullGoalIssue jiraIssue) {
        try {
...
//        return fullGoalClient.getAttachmentContent(fileKey);
//    }
//
...
//                if (!msFileNames.contains(filename) && !fullGoalImageFileNames.contains(filename)) {
//                    String fileId = attachment.get("id").toString();
//                    fullGoalClient.deleteAttachment(fileId);
//                }
//            }
...
//                fullGoalClient.uploadAttachment(request.getPlatformId(), img);
//            }
//        });
//    }
//
...
//                fullGoalImageFileNames.add(filename);
//            }
...
//
//            JiraIssueListResponse result = fullGoalClient.getProjectIssues(startAt, maxResults, jiraKey, projectConfig.getJiraIssueTypeId());
//            jiraIssues = result.getIssues();
//
//            currentSize = jiraIssues.size();
//            List<String> allIds = jiraIssues.stream().map(FullGoalIssue::getId).collect(Collectors.toList());
//            // 创建的时候是 platform_id 存的key，之前全量同步存的是id，统一改成存key，这里做兼容处理
//            allIds.addAll(jiraIssues.stream().map(FullGoalIssue::getKey).collect(Collectors.toList()));
//
...
//                    // 如果不包含附件信息，则查询下附件
//                    try {
//                        JiraIssueListResponse response = fullGoalClient.getProjectIssuesAttachment(startAt, maxResults,
//                                jiraKey, projectConfig.getJiraIssueTypeId());
//                        List<FullGoalIssue> jiraIssuesWithAttachment = response.getIssues();
//                        attachmentMap = jiraIssuesWithAttachment.stream()
//                                .collect(Collectors.toMap(FullGoalIssue::getKey,
...
//                }
//
//                for (FullGoalIssue jiraIssue : jiraIssues) {
...
//    }
//
//    private List<FullGoalIssue> filterSyncJiraIssueByCreated(List<FullGoalIssue> jiraIssues, SyncAllIssuesRequest syncRequest) {
//        List<FullGoalIssue> filterIssues = jiraIssues.stream().filter(jiraIssue -> {
